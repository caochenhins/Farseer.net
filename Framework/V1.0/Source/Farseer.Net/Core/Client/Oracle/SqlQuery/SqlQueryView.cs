...
namespace FS.Core.Client.Oracle.SqlQuery
{
    public class SqlQueryView<TEntity> : ISqlQueryView<TEntity> where TEntity : class,new()
    {
        ...
        public virtual void ToList(int top = 0, bool isDistinct = false, bool isRand = false)
        {
            ...
            {
                Queue.Sql.AppendFormat("SELECT {0} {1} FROM {2} {3} ORDER BY dbms_random.value {4}", strDistinctSql, strSelectSql, Query.DbProvider.KeywordAegis(TableName), strWhereSql, strTopSql);
            }
            else
            {
                Queue.Sql.AppendFormat("SELECT * FROM (SELECT {0} {1} FROM {2} {3} ORDER BY dbms_random.value {5}) a {4}", strDistinctSql, strSelectSql, Query.DbProvider.KeywordAegis(TableName), strWhereSql, strOrderBySql, strTopSql);
            }
        }
