...
namespace FS.Utils.Open
{
    ...
    public class QQ
    {
        ...
        public QQ(string appID, string appKey, string authorizationCode, string redirectUri)
        {
            AppID = appID;
            AppKey = appKey;
        ...
        {
            if (!string.IsNullOrWhiteSpace(OpenID)) { return OpenID; }
        ...
        public string API(string apiName, string paras, string method = "GET")
        {
            ...
            if (method.ToUpper() == "GET") { return WebForm.Net.Get(url + param); }

            return WebForm.Net.Post(url, param);
