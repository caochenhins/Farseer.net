...
namespace Farseer.Net.Core.Tests.TableTest
{
    ...
    public class TableSetTest
    {
        ...
        public void ToInfoTestMethod()
        {
            ...
            var info = Table.Data.User.Select(o => o.ID).Select(o => o.LoginCount).Where(o => o.ID > 1).ToInfo();
            ...
            info = Table.Data.User.Select(o => new { o.ID, o.PassWord }).ToInfo();
            Assert.IsNotNull(info);
            Assert.IsTrue(info.PassWord != null && info.GenderType == null && info.LoginIP == null && info.UserName == null && info.ID != null && info.LoginCount == null);
            ...
            info = Table.Data.User.Select(select).ToInfo();
            Assert.IsNotNull(info);
            Assert.IsTrue(info.PassWord != null && info.GenderType == null && info.LoginIP == null && info.UserName == null && info.ID != null && info.LoginCount == null);
            Assert.IsTrue(info.ID == lst[0].ID);
        ...
        {
            var lst = Table.Data.User.Desc(o => o.ID).ToList(10, true, true);
            lst = Table.Data.User.ToList(0, true, true);
            lst = Table.Data.User.ToList();
            Assert.IsTrue(lst != null && lst.Count > 0);
            var ID = lst[0].ID.GetValueOrDefault();

            lst = Table.Data.User.Select(o => new { o.ID, o.PassWord, o.GetDate }).Where(o => o.ID == ID).Desc(o => new { o.LoginCount, o.GenderType }).Asc(o => o.ID).Desc(o => o.GetDate).ToList();
            var info = lst[0];
            ...
            lst = Table.Data.User.ToList(3);
            Assert.IsNotNull(lst);
            Assert.IsTrue(lst.Count <= 3);

            lst = Table.Data.User.ToList(3, 2);
            Assert.IsNotNull(lst);
            Assert.IsTrue(lst.Count <= 3);

            var count = Table.Data.User.Where(o => o.ID > 10).Count();
            var recordCount = 0;
            lst = Table.Data.User.Where(o => o.ID > 10).ToList(99999, 1, out  recordCount).ToList();
            Assert.IsNotNull(lst);
        ...
        public void InsertTestMethod()
        {
            ...
            info = Table.Data.User.Desc(o => o.ID).ToInfo();
